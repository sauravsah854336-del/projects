...
const SectionHeader = ({ title, subtitle, gradientFrom = "from-orange-50", extra }) => (
  <div className={`px-5 sm:px-7 py-5 border-b border-gray-100 bg-gradient-to-r ${gradientFrom} to-white flex items-center justify-between gap-4`}>
    <div>
...
const ProfilePage = () => {
  ...
  const handleAvatarUpload = async (e) => {
    ...
    if (!["image/jpeg","image/jpg","image/png","image/webp"].includes(file.type)) {
      setProfileError("Only JPG, PNG, or WebP images allowed"); return;
    }
    ...
    } catch { setProfileError("Upload failed. Please try again."); }
    finally { setAvatarUploading(false); }
  };

  const handleProfileSubmit = async (e) => {
    e.preventDefault(); setProfileMsg(null); setProfileError("");
    if (!currentProfile.firstName.trim()) { setProfileError("First name is required"); return; }
    try {
      ...
      setProfileForm(null);
    } catch (err) { setProfileError(err?.data?.message || "Failed to update profile"); }
  };
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    } catch (err) { setPasswordError(err?.data?.message || "Failed to change password"); }
  };

  const openAddAddress = () => {
    setAddressForm(EMPTY_ADDRESS); setEditingAddressId(null);
    setAddressError(""); setAddressMsg(null); setShowAddressForm(true);
  };
  ...
  const openEditAddress = (address) => {
    setAddressForm({
      ...
      country: address.country || "India", isDefault: address.isDefault || false,
    });
    setEditingAddressId(address._id); setAddressError(""); setAddressMsg(null); setShowAddressForm(true);
  };

  const handleAddressSubmit = async (e) => {
    e.preventDefault(); setAddressMsg(null); setAddressError("");
    ...
    if (!/^[6-9]\d{9}$/.test(phone.trim())) { setAddressError("Enter a valid 10-digit Indian phone number"); return; }
    if (postalCode.trim().length !== 6) { setAddressError("PIN code must be 6 digits"); return; }
    try {
      if (editingAddressId) {
      ...
      setShowAddressForm(false); setEditingAddressId(null); setAddressForm(EMPTY_ADDRESS);
    } catch (err) { setAddressError(err?.data?.message || "Failed to save address"); }
  };

  const handleDeleteAddress = async (addressId) => {
    try { await deleteAddress(addressId).unwrap(); setDeletingAddressId(null); }
    catch (err) { console.log(err); }
  };

  const handleSetDefault = async (addressId) => {
    try { await setDefaultAddress(addressId).unwrap(); }
    catch (err) { console.log(err); }
  };
  ...
  return (
    <div className="bg-gray-100 min-h-screen py-5 sm:py-7 px-3 sm:px-4">
      <div className="max-w-[1000px] mx-auto">
        ...
        <Link to="/dashboard" className="inline-flex items-center gap-1.5 text-gray-500 text-[13px] font-semibold no-underline mb-5 hover:text-gray-700 transition-colors">
          ← Back to Dashboard
        </Link>
        ...
        <div className="flex flex-col xl:grid xl:grid-cols-[260px_1fr] gap-4 items-start">
          ...
          <div className="flex flex-col gap-3 xl:sticky xl:top-20 w-full">
            <div className="bg-white rounded-2xl border border-gray-200 p-5 sm:p-6 text-center">
              <div className="relative w-[86px] h-[86px] mx-auto mb-3.5">
                <div className="w-[86px] h-[86px] rounded-full bg-gradient-to-br from-[#D85A30] to-[#FF8C5A] flex items-center justify-center overflow-hidden border-4 border-white shadow-lg shadow-orange-500/20">
                  {currentProfile.avatar ? (
                    <img src={currentProfile.avatar} alt="Avatar" className="w-full h-full object-cover" onError={(e) => { e.target.style.display = "none"; }} />
                  ) : (
                    <span className="text-[34px] font-black text-white">{profile?.firstName?.[0]?.toUpperCase() || "U"}</span>
                  )}
                </div>
              ...
              <p className="text-base font-black text-gray-900 m-0">{profile?.firstName} {profile?.lastName}</p>
              ...
              {navItems.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-3 w-full px-4 py-3 rounded-xl border-none cursor-pointer text-left font-[inherit] transition-all ${active ? "bg-orange-50 text-[#D85A30]" : "bg-transparent text-gray-700 hover:bg-gray-100"}`}
                  >
                    <span className="text-lg w-6 text-center">{item.icon}</span>
                    <span className={`text-sm flex-1 ${active ? "font-extrabold text-[#D85A30]" : "font-medium"}`}>{item.label}</span>
            ...
            <Link to="/orders" className="bg-white border border-gray-200 rounded-2xl p-3.5 no-underline flex items-center gap-3 hover:border-[#D85A30] hover:bg-orange-50 transition-all">
          ...
          <div className="w-full">
                ...
                <form onSubmit={handleProfileSubmit} className="p-5 sm:p-7">
                  ...
                  <div className="mb-4">
                    ...
                    <div className="flex items-center gap-2.5 w-full border-[1.5px] border-gray-200 rounded-xl px-3.5 py-3 bg-gray-100 text-gray-400 cursor-not-allowed text-sm">
                      ...
                      <span className="text-[11px] bg-gray-200 text-gray-500 px-2 py-0.5 rounded-full">Cannot be changed</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3">
                    <button
                      ...
                      className={`flex items-center gap-2 px-7 py-3 rounded-xl text-sm font-extrabold text-white border-none cursor-pointer transition-all ${updatingProfile ? "bg-gray-400 cursor-not-allowed" : "bg-gradient-to-r from-[#D85A30] to-[#FF8C5A] shadow-lg shadow-orange-500/25 hover:shadow-orange-500/40"}`}
                    >
                      {updatingProfile && <span className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin"></span>}
            ...
            {activeSection === "password" && (
              <div className="animate-in fade-in bg-white rounded-2xl border border-gray-200 overflow-hidden">
                <SectionHeader title="Change Password" subtitle="Keep your account secure with a strong password" gradientFrom="from-blue-50" />
                <form onSubmit={handlePasswordSubmit} className="p-5 sm:p-7">
                    ...
                    {[
                      ...
                      <div key={item.key}>
                          ...
                          <div className="mt-2.5 p-3.5 bg-gray-50 rounded-xl border border-gray-200">
                            <div className="flex items-center gap-2 mb-2">
                              <div className="flex-1 h-[5px] bg-gray-200 rounded-full overflow-hidden">
                                <div className={`h-full ${strength.color} ${strength.width} rounded-full transition-all duration-300`}></div>
                              </div>
                              <span className={`text-[11px] font-extrabold ${strength.textColor} min-w-[50px]`}>{strength.text}</span>
                            </div>
                        ...
                        {item.key === "confirm" && passwordForm.confirmPassword && (
                          <p className={`text-xs mt-1.5 flex items-center gap-1 font-bold ${passwordForm.newPassword === passwordForm.confirmPassword ? "text-green-500" : "text-red-500"}`}>
                  ...
                  <button
                    ...
                    className={`mt-6 flex items-center gap-2 px-7 py-3 rounded-xl text-sm font-extrabold text-white border-none cursor-pointer transition-all ${changingPassword ? "bg-gray-400 cursor-not-allowed" : "bg-gray-900 hover:bg-gray-800"}`}
                  >
            ...
            {activeSection === "addresses" && (
              <div className="animate-in fade-in">
                <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
                  <SectionHeader
                    title="Address Book"
                    subtitle={`${profile?.addresses?.length || 0} saved ${profile?.addresses?.length === 1 ? "address" : "addresses"}`}
                    gradientFrom="from-green-50"
                    extra={!showAddressForm && (
                      <button
                  ...
                  {addressMsg && <div className="px-5 sm:px-7 pt-4"><Alert type={addressMsg.type} text={addressMsg.text} /></div>}
                      ...
                      <form onSubmit={handleAddressSubmit}>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 mb-3.5">
                          <div>
                            ...
                            <Input placeholder="John Doe" value={addressForm.fullName} onChange={(e) => setAddressForm((p) => ({ ...p, fullName: e.target.value }))} />
                          </div>
                          <div>
                            ...
                            <div className="relative">
                              ...
                              <Input className="pl-[54px]" placeholder="9876543210" value={addressForm.phone} onChange={(e) => setAddressForm((p) => ({ ...p, phone: e.target.value }))} maxLength={10} />
                            </div>
                          </div>
                        </div>
                        <div className="mb-3.5">
                          <Label required>Street Address</Label>
                          <Input placeholder="House No, Building, Street, Area, Landmark" value={addressForm.street} onChange={(e) => setAddressForm((p) => ({ ...p, street: e.target.value }))} />
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3.5 mb-3.5">
                          <div>
                            <Label required>City</Label>
                            <Input placeholder="Mumbai" value={addressForm.city} onChange={(e) => setAddressForm((p) => ({ ...p, city: e.target.value }))} />
                          </div>
                          <div>
                          ...
                          <div>
                            ...
                            <Input placeholder="400001" value={addressForm.postalCode} onChange={(e) => setAddressForm((p) => ({ ...p, postalCode: e.target.value }))} maxLength={6} className="font-mono tracking-widest font-semibold" />
                          </div>
                        </div>
                        <label className="flex items-center gap-2.5 cursor-pointer mb-5">
                          ...
                          />
                          <span className="text-[13px] text-gray-700 font-semibold">Set as my default delivery address</span>
                        </label>
                        <div className="flex gap-2.5">
                          ...
                          <button
                            ...
                            className={`flex-1 py-3 rounded-xl text-sm font-extrabold text-white border-none cursor-pointer font-[inherit] transition-all ${addingAddress || updatingAddress ? "bg-gray-400 cursor-not-allowed" : "bg-gradient-to-r from-[#D85A30] to-[#FF8C5A]"}`}
                          >
                  ...
                  <div className="p-5 sm:p-7">
                    {profile?.addresses?.length === 0 && !showAddressForm && (
                      <div className="text-center py-10 px-5 bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200">
                        ...
                        <p className="text-[15px] font-extrabold text-gray-700 m-0">No saved addresses</p>
                        <p className="text-[13px] text-gray-500 mt-1 mb-4">Add a delivery address to speed up checkout</p>
                        <button
                          onClick={openAddAddress}
                    ...
                    <div className="flex flex-col gap-3">
                      {profile?.addresses?.map((address) => (
                        <div
                          key={address._id}
                          className={`rounded-2xl p-4 sm:p-5 border-[1.5px] transition-all ${address.isDefault ? "border-[#D85A30] bg-gradient-to-br from-orange-50 to-orange-50/30" : "border-gray-200 bg-white"}`}
                        >
                          <div className="flex items-start justify-between gap-3.5">
                            <div className="flex-1">
                              <div className="flex items-center gap-2 mb-2">
                                <p className="text-sm font-black text-gray-900 m-0">{address.fullName}</p>
                                {address.isDefault && (
                                  <span className="text-[10px] bg-[#D85A30] text-white px-2.5 py-0.5 rounded-full font-black">✓ Default</span>
                                )}
                              </div>
                              <p className="text-[13px] text-gray-700 m-0 mb-0.5 leading-relaxed">{address.street}</p>
                              <p className="text-[13px] text-gray-700 m-0 mb-0.5">{address.city}, {address.state} — {address.postalCode}</p>
                            ...
                            <div className="flex flex-col gap-1.5 shrink-0">
                              <button onClick={() => openEditAddress(address)} className="bg-purple-100 text-purple-800 border border-purple-300 rounded-lg px-3.5 py-1.5 text-xs font-bold cursor-pointer hover:bg-purple-200 transition font-[inherit]">
                                ✏️ Edit
                              </button>
                              {!address.isDefault && (
                                <button onClick={() => handleSetDefault(address._id)} className="bg-orange-50 text-[#D85A30] border border-orange-200 rounded-lg px-3.5 py-1.5 text-xs font-bold cursor-pointer hover:bg-orange-100 transition font-[inherit]">
                                  Set Default
                                </button>
                              )}
                              {deletingAddressId === address._id ? (
                                <div className="flex gap-1">
                                  <button onClick={() => handleDeleteAddress(address._id)} className="flex-1 bg-red-500 text-white border-none rounded-lg py-1.5 px-2.5 text-xs font-bold cursor-pointer font-[inherit]">Yes</button>
                                  <button onClick={() => setDeletingAddressId(null)} className="flex-1 bg-white text-gray-700 border border-gray-200 rounded-lg py-1.5 px-2.5 text-xs cursor-pointer font-[inherit]">No</button>
                                </div>
                              ) : (
                                <button onClick={() => setDeletingAddressId(address._id)} className="bg-red-50 text-red-900 border border-red-200 rounded-lg px-3.5 py-1.5 text-xs font-bold cursor-pointer hover:bg-red-100 transition font-[inherit]">
                                  🗑️ Delete
                                </button>
                              )}
                            </div>
