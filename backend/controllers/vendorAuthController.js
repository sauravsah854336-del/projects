...
const vendorSignup = async (req, res) => {
  try {
    ...
    if (!ifscRegex.test(bankDetails.ifscCode.trim().toUpperCase())) {
      return res.status(400).json({ success: false, message: "Valid IFSC code is required (e.g. SBIN0001234)" });
    }

    if (bankDetails.accountNumber.trim().length < 9) {
      return res.status(400).json({ success: false, message: "Valid bank account number is required (minimum 9 digits)" });
    }
    ...
    if (stateCode < 1 || stateCode > 37) {
      return res.status(400).json({ success: false, message: "Invalid state code in GST number" });
    }
    ...
    if (existStore) {
      return res.status(409).json({ success: false, message: "Store name is already taken. Please choose a different name." });
    }
    ...
    if (existPAN) {
      return res.status(409).json({ success: false, message: "This PAN number is already registered with another vendor" });
    }
    ...
    if (existGST) {
      return res.status(409).json({ success: false, message: "This GST number is already registered with another vendor" });
    }
  ...
  try {
    ...
    if (vendor.approvalStatus === "pending") {
      return res.status(403).json({
        success: false,
        message: "Your account is pending admin approval. We will notify you within 24-48 hours.",
      });
    }
...
const updateVendorStore = async (req, res) => {
  try {
    const {
      ...
      bankDetails, businessAddress, warehouseAddress,
    } = req.body;
    ...
    if (storeDescription !== undefined) vendor.storeDescription = storeDescription.trim();
